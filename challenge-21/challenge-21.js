...
//Outra forma de criar função de evento:
/*$start.addEventListener('click', function startTimer() {
	aqui entra a função do startTimer;
}, false); */       // a função entra como parâmetro do evento
...
function startTimer() {
	$timer.value = +$timer.value +1; //utilizar + antes de uma de string (unário) a converte em número.
...
function resetTimer(){
	$timer.value = 0;
	stopTimer(); // Ou clearTimeout(interval);
}
