...
//Ou para evitar repetições de console.log.... 
...
//simplificando exemplo anterior... (método indicado) 

console.log( '\nFormatando CPFs corretamente(simplificado):' );
cpfs.forEach( function ( cpf ) {
	console.log( cleanCPF( cpf ).replace( /(\d{3})(\d{3})(\d{3})(\d{2})/, '$1.$2.$3-$4' ) );
} );

//Ou também podemos solucionar utilizando uma função:

console.log( '\nFormatando CPFs corretamente(com função):' );
cpfs.forEach( function ( cpf ) {
...
/*forma alternativa: .replace( /(<(\w+)>)([^<]+)(<\/\w+>)/g, 
