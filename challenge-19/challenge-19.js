...
linha, independent e de quantos caracteres de número estiverem juntos.
...
function hasClass( markup, cssClass ){
	var regex = new RegExp( 'class=["\'].*'+ cssClass +'.*["\']' ); //regex reduzida a partir do mais longo
	return regex.test( markup ); 

	//regex do exemplo anterior mais longo: class=["\'](?:[\\w\\s]+)?'+ cssClass +'(?:[\\w\\s]+)?["\']
}
